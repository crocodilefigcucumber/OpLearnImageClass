...
class CNO2dClassifier(nn.Module):
    def __init__(
        self,
        in_dim,  # Number of input channels.
        ...
        super(CNO2dClassifier, self).__init__()
        ...
        self.CNO = CNO2d(
            in_dim=in_dim,  # Number of input channels.
            out_dim=1,  # Number of output channels.
            size=size,  # Input and Output spatial size (required )
    ...
    def name(self):
        return f"CNO-N_layers{self.N_layers}-N_res{self.N_res}-N_res_neck{self.N_res_neck}-channel_mult{self.channel_multiplier}"
